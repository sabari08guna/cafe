...
function Songs() {
  ...
  // Fetch favorities items
      axios.get('http://localhost:5000/favorities')
  ...
  const addToWishlist = async (itemId) => {
    try {
      ...
      await axios.post('http://localhost:5000/favorities', { itemId: itemId2, title, imgUrl, genre, songUrl, singer });
      const response = await axios.get('http://localhost:5000/favorities');
  ...
  const removeFromWishlist = async (itemId) => {
    try {
      ...
      await axios.delete(`http://localhost:5000/favorities/${selectedItem.id}`);
  ...
  const addToPlaylist = async (itemId) => {
    try {
      ...
      const { title, imgUrl, genre, songUrl, singer, id: itemId2 } = selectedItem;
      await axios.post('http://localhost:5000/playlist', { itemId: itemId2, title, imgUrl, genre, songUrl, singer });
  ...
  const removeFromPlaylist = async (itemId) => {
    try {
      ...
      // Make a DELETE request to remove the item from the wishlist
      await axios.delete(`http://localhost:5000/playlist/${selectedItem.id}`);
      // Refresh the wishlist items
      const response = await axios.get('http://localhost:5000/playlist');
      setPlaylist(response.data);
    } catch (error) {
  ...
  const filteredItems = items.filter((item) => {
    ...
    return (
      <div style={{display:"flex",marginLeft:"300px", justifyContent:"flex-end"}}>
